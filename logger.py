...
class CsvLogger:
    def __init__(self, filepath='./', filename='results.csv', data=None):
        ...
        self.fieldsnames = ['epoch', 'test_mae', 'test_loss', 'val_mae', 'val_loss', 'train_mae', 'train_loss']
    ...
    def plot_progress_mae(self, title='STN MobileNetv2 MAE'):
        plt.figure(figsize=(9, 8), dpi=300)
        plt.plot(self.data['train_mae'], label='Training')
        plt.plot(self.data['val_mae'], label='Validation')
        plt.title(title)
        plt.xlabel('Epoch')
        plt.ylabel('MAE')
        plt.legend()
        plt.xlim(0, len(self.data['train_mae']) + 1)
        plt.savefig(os.path.join(self.log_path, 'mae.png'))

    def plot_progress_loss(self, title='MobileNetv2 Loss'):
        plt.figure(figsize=(9, 8), dpi=300)
